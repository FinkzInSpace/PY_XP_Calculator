...
def calculate_xp(config: xpConfig, session: sessionXpInput) -> int:
  ...
  """
  encounter_xp = (
    (session.easy_encounters * config.xp_easy) + 
    (session.moderate_encounters * config.xp_moderate) +
    (session.hard_encounters * config.xp_hard) + 
    (session.impossible_encounters * config.xp_impossible)
  )
  lore_xp = (
    (session.new_lore * config.xp_new_lore) +
    (session.enhanced_lore * config.xp_enhanced_lore)
  )
  quest_xp = (
    (session.side_quests_completed * config.xp_per_side_quest) +
    (config.xp_followed_plot if session.followed_plot else 0)
  )
